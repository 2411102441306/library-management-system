        ...
        <div class="bg-white rounded-xl border border-slate-200 p-5 hover:border-sky-200 transition-all">
            <div class="flex items-start justify-between mb-3">
                <div class="flex items-center gap-3">
                    <div class="w-10 h-10 rounded-full flex items-center justify-center text-sm font-semibold flex-shrink-0" style="background:#E0F2FE;color:#0EA5E9">
            ...
            <div class="flex items-center justify-between text-xs text-slate-500 mb-4 py-3 border-t border-b border-slate-100">
                <div>
                    ...
                    <div class="font-medium text-slate-700 mt-0.5">{{ $member->created_at->format('d M Y') }}</div>
                </div>
            ...
            <div class="flex items-center gap-2">
                <a href="{{ route('admin.members.show', $member) }}" class="flex-1 text-center py-2 rounded-lg border text-xs font-medium transition-all hover:bg-sky-50" style="border-color:#0EA5E9;color:#0EA5E9">
                    <i class="ti ti-eye mr-1"></i> Lihat Detail
                </a>
                <a href="{{ route('admin.members.edit', $member) }}" class="flex-1 text-center py-2 rounded-lg border border-slate-200 text-xs font-medium text-slate-600 hover:bg-slate-50 transition-all">
